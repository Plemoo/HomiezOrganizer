...
import { getAllDocumentsOfCollection, getFirebaseDocumentArray } from "../firebaseExchange"
...
export const getAllActivitiesByGroupIds = (groupIds: string[]): Promise<{ group: IGroup, activities: IActivity[] }[]> => {
    return getFirebaseDocumentArray(groupIds, "Group")
        ...
        .then((groups) => {
            const activityPromises = groups.map((group) => getAllDocumentsOfCollection("Group", group.id, "Activity"));
